...
public enum MemorySegment: Sendable {
...
public extension MemorySegment {

	@inlinable static func clear(storage: inout [UInt16]) {
		storage.removeAll(keepingCapacity: true)
	}

	@inlinable static func read(from storage: [UInt16], at index: UInt16) -> UInt16 {
		let windex = Int(index)
		guard windex < storage.count else { return 0 }
		return storage[windex]
	}

	@inlinable static func write(_ newValue: UInt16, to storage: inout [UInt16], at index: UInt16) {
		let windex = Int(index)
